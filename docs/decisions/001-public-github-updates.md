...
- 교체는 같은 볼륨에서 디렉터리 rename으로 수행한다. 이전 버전 하나를 backup으로 유지하고,
  기동 검증이 끝난 뒤에만 성공으로 기록한다. 성공이 durable하게 기록된 뒤 backup을 삭제하며,
  파일 잠금으로 즉시 삭제할 수 없으면 candidate에 결속된 숨김 sibling으로 옮겨 다음 실행에서 재시도한다.
  이 정리 실패만으로 정상 기동한 새 버전을 rollback하지 않는다.
