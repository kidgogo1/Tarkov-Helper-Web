...
- 앱 시작 시 공개 GitHub의 최신 stable 릴리스를 확인하고, `설정 > 데이터 > 프로그램 업데이트`에서
  사용자가 다시 확인할 수 있게 한다.
- 사용자가 선택한 정확한 후보만 백그라운드에서 다운로드하고 검증한다. 실행 중인 폴더는 건드리지
  않으며, 검증이 끝나면 다음 실행을 기다린다.
- 사용자가 탭을 닫아 로컬 서버가 종료된 다음 다시 실행하면 외부 broker가 전체 배포 폴더를
  교체한다. 새 서버의 버전·빌드 identity와 health check가 실패하면 이전 폴더로 자동 롤백한다.
