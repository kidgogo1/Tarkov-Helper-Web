...
Dim shell, fileSystem, processEnvironment, packageRoot, runtimeRoot, powerShellPath, powerShellCommand, command, exitCode
...
If Not fileSystem.FileExists(powerShellPath) Then
  ...
  WScript.Quit 1
End If
powerShellCommand = "& ([IO.Path]::Combine($env:TARKOV_HELPER_PACKAGE_ROOT, 'start-menu.ps1')) -Action Unregister"
command = Quote(powerShellPath) & " -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command " & Quote(powerShellCommand)
exitCode = shell.Run(command, 0, True)

If exitCode = 0 Then
  MsgBox "Tarkov Helper was removed from the Windows Start menu.", 64, "Tarkov Helper"
Else
  MsgBox "Tarkov Helper could not be removed from the Windows Start menu.", 16, "Tarkov Helper"
End If
