...
v1.0.42의 변경사항과 사용 시 주의점은 [릴리즈 안내](https://github.com/kidgogo1/Tarkov-Helper-Web/blob/v1.0.42/docs/releases/v1.0.42.md)를 참고하세요.
...
아이콘이 내장된 `Tarkov Helper.exe`를 더블클릭하면 Windows Script Host에 의존하지 않고 콘솔 창 없이
백그라운드 서버와 기본 브라우저가 열립니다.
이 파일을 오른쪽 클릭하면 Windows의 `작업 표시줄에 고정` 기능도 사용할 수 있습니다. EXE 실행이 보안 정책으로
차단되고 Windows Script Host는 허용된 환경에서만 `Tarkov Helper 실행.vbs`를 호환용 실행 파일로 사용할 수 있습니다.
...
검증된 업데이트 후보가 표시된 동안에는 같은 확인을 반복해 GitHub 요청 제한을 소모하지 않고 그 후보를
그대로 다운로드·적용합니다. 적용 직전 폴더 잠금이 계속되면 검증된 다운로드를 지우지 않고 이전 버전을
다시 시작해 `지금 적용하고 계속 사용`으로 재시도할 수 있게 합니다.
...
v1.0.5 이상은 사용 중 한 번의 클릭으로 적용·재연결할 수 있습니다. v1.0.3과 v1.0.4는 업데이트를
다운로드한 뒤 앱을 종료하고 다시 실행하면 최신 버전을 적용합니다. v1.0.1과 v1.0.2는 실행 방식에 따라
구버전 프로세스가 자기 폴더를 잠글 수 있으므로 자동 적용을 반복하지 말고 최신 Direct ZIP을 받아야 합니다.
또한 v1.0.20 이하를 회사 프록시에서 사용하거나 어떤 버전이든 업데이트가 반복 실패하면 같은 수동 절차를
사용하세요.
기존 앱을 먼저 종료하고, 기존 폴더 위에 덮어쓰지 말고 `%USERPROFILE%\TarkovHelper`처럼 짧은 새 폴더에
압축을 푼 뒤 실행하세요. 새 버전이 정상 실행되는 것을 확인할 때까지 기존 폴더를 보관하면 수동 복구도
가능합니다. 진행도와 설정은 배포 폴더 밖의 고정 주소 브라우저 저장소에 있으므로 같은 브라우저 프로필과
`127.0.0.1:41753`을 유지하면 그대로 남습니다.

공유 런타임 상태 자체가 손상되어 새 폴더의 최신 버전까지 시작하지 못하고 `Tarkov Helper 상태 복구.cmd`도
안전을 위해 거부한 경우에만, 기존 앱을 완전히 종료하고 41753 포트를 비운 뒤 새 폴더의
`Tarkov Helper 격리 복구 실행.cmd`를 실행하세요. 이 명령은 일반 상태를 삭제하거나 옮기지 않고
`%LOCALAPPDATA%\TarkovHelperWeb-Isolated-Recovery`만 사용하는 명시적 탈출구입니다. 서버 주소는 그대로라
같은 브라우저 프로필의 진행도는 유지됩니다. 격리 실행을 끝낼 때는 그 새 폴더의 명령 프롬프트에서
`"Tarkov Helper 격리 복구 실행.cmd" stop`을 실행해야 하며, 일반 `Tarkov Helper 종료.vbs`와 상태가
분리되어 있습니다. 격리 모드는 새 폴더에 이미 들어 있는 앱 파일만 읽기 전용으로 제공하며, 업데이트
확인(Check)·준비(Stage)·적용(Apply)과 패키지 폴더 옆의 rollback/cleanup 정리를 모두 비활성화합니다.
원인 확인을 위해 일반 상태와 기존 패키지는 그대로 보존하세요.

오류 화면의 `지원 코드`와 `설정 > 데이터 > 진단 기록`을 함께 확인하면 네트워크, GitHub 요청 제한,
서명·패키지 검증, 적용·복원 중 어느 단계에서 실패했는지 구분할 수 있습니다.
...
최초 공개 릴리스 전에는 같은 값을 `release.config.example.json`의 `trustedKeyId`에 고정합니다. 이후
Repository variable과 Environment secret을 함께 바꾸더라도 이 신뢰 루트가 다르면 릴리스가 중단됩니다.
기존 설치본을 위한 별도 bridge 계획 없이 이 값을 변경하지 마세요.
...
[ADR-001](./docs/decisions/001-public-github-updates.md), 구버전·다중 PC 호환성은
[ADR-004](./docs/decisions/004-distributed-upgrade-compatibility.md)를 참고하세요.
